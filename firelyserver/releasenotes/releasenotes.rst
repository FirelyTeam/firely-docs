...
Fix
^^^
#. Fixed a bug when trying to delete multiple resources at once (bulk delete, see :ref:`_restful_crud_configuration` for configuration options). The operation would take a while and eventually return a ``204 No Content`` without actually deleting any resources. This is now fixed, the bulk delete operation returns much faster and the resources are deleted.
